...
const updateNote = () => { if (!editingContent.value.trim() || !activeTitle.value) return; const list = notesCache.value[activeTitle.value]; const index = list.findIndex(n => n.id === editingNoteId.value); if (index !== -1) { list[index].content = editingContent.value; saveNotes(activeTitle.value); } cancelEdit() }
const deleteNote = (id) => { if (!activeTitle.value) return; notesCache.value[activeTitle.value] = notesCache.value[activeTitle.value].filter(n => n.id !== id); saveNotes(activeTitle.value); }
